            ...
            using(ImRaii.PushColor(ImGuiCol.Button, UIConstants.Transparent))
            {
                ...
                static void ShowTransformTooltip(string type) {
                    using(ImRaii.Tooltip()) {
                        ImGui.Text($"导入 {type}");
                        ImGui.TextDisabled("不影响表情导入。");
                    }
                }
                
                if(ImBrio.ToggelFontIconButton("ImportPosition", FontAwesomeIcon.ArrowsUpDownLeftRight, buttonSize, transformComponents.Value.HasFlag(TransformComponents.Position), hoverAction: () => ShowTransformTooltip("位置")))
                {
                ...
                ImGui.SameLine();
                if(ImBrio.ToggelFontIconButton("ImportRotation", FontAwesomeIcon.ArrowsSpin, buttonSize, transformComponents.Value.HasFlag(TransformComponents.Rotation), hoverAction: () => ShowTransformTooltip("旋转")))
                {
                ...
                ImGui.SameLine();
                if(ImBrio.ToggelFontIconButton("ImportScale", FontAwesomeIcon.ExpandAlt, buttonSize, transformComponents.Value.HasFlag(TransformComponents.Scale), hoverAction: () => ShowTransformTooltip("缩放")))
                {
        ...
        LibraryManager.GetWithFilePicker(filter, (r) =>
        {
            ...
            else if(r is LivePoseFile livePose)
            {
                if(LivePose.TryGetService<IpcService>(out var ipcService)) {
                    ...
                    capability.SkeletonPosing.PoseInfo = poseInfo.Clone(asExpression ? capability.SkeletonPosing.FilterFaceBones : capability.SkeletonPosing.FilterBodyBones);
